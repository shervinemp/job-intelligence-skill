...
(730 passed + 141 subtests). Generated with:
...
**Production coverage: 47.5%** (14,252 statements). Raised from 42.3% across
six passes:
...
- `apply/common/registry_cli.py` 9.7% → 42.5% · `enrich.py` 15.2% → 20.2%
...
8. `test_probe_cascade.py` — the probe cascade via jsdom CorpusPage: strategy
   ...
   (8 tests).**
9. `test_registry_and_stage_cli.py` — the registry CLI dispatch (confirm/clear
   require hash, prefix resolution, empty-state messages) and the enrich
   flag/status commands against a temp DB. **Added this pass (11 tests).**
