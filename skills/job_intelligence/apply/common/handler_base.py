...
ΓöÇΓöÇ How to add a new platform ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
4. That's it. The run_modal_flow() runner loops detect ΓåÆ fill ΓåÆ advance.
   No changes needed to act.py, registry.py, or any other file.

ΓöÇΓöÇ What each method should do ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

  detect(page)         ΓåÆ Full page snapshot: fields, buttons, errors, progress.
  classify(page)       ΓåÆ Quick string: 'form' | 'review' | 'success' | 'login'.

  extract_fields(page) ΓåÆ Visible fillable fields with labels.
  fill(page, field, v) ΓåÆ Set field using the right framework setter.
  upload(page, f, p)   ΓåÆ File upload (widget-specific).

  can_proceed(page)    ΓåÆ True if a next/submit button exists & is enabled.
  click_next(page)     ΓåÆ Click next/continue. Return navigated=True if URL changed.
  click_submit(page)   ΓåÆ Click submit. Return navigated=True if dialog closed.
  ensure_modal_open(p) ΓåÆ Open the form overlay if closed (for MODAL flow_type).

  ensure_resume(p, jid)ΓåÆ Select or upload the tailored resume. Return False to block.
  is_applied(page)     ΓåÆ True if success signal detected.
  get_errors(page)     ΓåÆ Validation error messages on the current page.

ΓöÇΓöÇ Shared utilities you can use ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
ΓöÇΓöÇ FlowType ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

  MODAL   ΓÇö Ephemeral overlay (LinkedIn). Runner re-opens if closed.
  PAGE    ΓÇö Multi-step form on sequential pages (Workday).
  SINGLE  ΓÇö One-page form, single submit (most ATS).
  REDIRECT ΓÇö Redirects to external URL (Greenhouse).
  MAILTO  ΓÇö Opens mail client (cannot automate).
  LOGIN_WALL ΓÇö Auth required before form.
"""
...
# ΓöÇΓöÇΓöÇ Enums ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ Dataclasses ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ Handler interface ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
class PlatformHandler(ABC):
    ...
    # ΓöÇΓöÇ Page state ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Field ops ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
    ...
    # ΓöÇΓöÇ Resume ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ

    @abstractmethod
    def ensure_resume(self, page, jid: str) -> bool:
        ...

    # ΓöÇΓöÇ Signals ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ JS helper: safe string interpolation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ Framework setters (shared) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ DOM traversal helpers (shared) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
# ΓöÇΓöÇΓöÇ Generic flow runner ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
def run_modal_flow(
    ...
    *,
    ...
    Loops: detect ΓåÆ fill required fields ΓåÆ try submit ΓåÆ try next ΓåÆ repeat.

    When dry_run=True: resolves all fieldΓåÆanswer mappings and prints them
    without modifying the DOM. Returns "paused" after preview.

    Returns:
        "done"    ΓÇö application submitted successfully
        "paused"  ΓÇö needs LLM input or manual action
        "failed"  ΓÇö cannot proceed
    """
    ...
    for step_idx in range(max_steps):
        ...
        # Preview: show all fieldΓåÆvalue mappings before any DOM changes.
        # Always prints ΓÇö both in dry-run and apply mode. In apply mode, proceeds
        # immediately after printing so the user can see what's about to happen.
        print(f"\n  ΓöÇΓöÇ FIELDS ({len(state.fields)} total) ΓöÇΓöÇ", file=sys.stderr)
        unfilled_count = 0
        for f in state.fields:
            if f.value:
                print(f"  Γ£ô [{f.type.name:6s}] {f.label[:45]:45s} = {f.value[:50]}", file=sys.stderr)
            else:
                res = resolution_for_fill(f.key, profile, answers_override=answers)
                val = res.value if res and res.value else ""
                if val:
                    print(f"  Γê╝ [{f.type.name:6s}] {f.label[:45]:45s} ΓåÆ {val[:50]} (from {'--answers' if res.provenance == 'user_typed' else res.provenance})", file=sys.stderr)
                else:
                    print(f"  ? [{f.type.name:6s}] {f.label[:45]:45s}  <-- needs your input", file=sys.stderr)
                    unfilled_count += 1

        if dry_run:
            emit_status("paused", "preview complete ΓÇö re-run with --apply to fill and submit")
            emit_next("act --fill --answers '{\"<label>\": \"<value>\"}' --apply")
            return "paused"

        # Fill unfilled required fields ΓÇö try profile match only (no guesses)
        filled_any = False
        for f in state.fields:
                ...
                if _clears:
                    ...
                    for _c in _clears[:2]:
                        emit_diag(_c.get("label", "?"), _c.get("oldVal", ""), "(cleared)", "re_render",
                                  "Backbone re-rendered after fill ΓÇö atomic mode needed")
            except Exception:
                pass

        # Don't pause on validation errors ΓÇö try advancing anyway.
        if state.errors:
        ...
        # Next ΓÇö Playwright's click() auto-waits for enabled + stable,
