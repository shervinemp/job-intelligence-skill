...
def cmd_fill(jid, answers_json=None, candidate=None, dry_run=False):
    ...
    modal = page.evaluate("() => !!document.querySelector('[role=\"dialog\"]')")
    if modal:
        ps = read_page(page)  # re-read to get modal fields
    else:
        try:
            is_easy = page.evaluate("""() => {
    ...
    if modal or (is_easy and ps.get("fieldCount", 0)):
        ...
        for _ in range(8):
            ...
            clicked = page.evaluate("""(kws) => {
                ...
                for (const kw of kws) {
                    for (const btn of dialog.querySelectorAll('button, a, [role="button"]')) {
