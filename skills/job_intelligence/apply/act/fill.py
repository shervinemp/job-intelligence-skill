...
def cmd_fill(jid, answers: dict = None, verify: bool = True, max_pages: int = 4,
    ...
    try:
        with chrome_session(state) as (page, ctx):
            ...
            # Post-login 2FA + CAPTCHA gates: some platforms (Workday, etc.)
            # show the 6-digit code prompt or a CAPTCHA only AFTER sign-in
            # succeeds — the mid-login heuristics can't see them. Lives in the
            # auth-flow module; True means a gate blocked and we stop.
            if post_login_gates(page, state, deadline):
                return 1
...
# ─── auth-wall machine (C1 deepening) ────────────────────────────────
# The login / account-creation / 2FA / CAPTCHA flow now lives in
# apply/act/auth_flow.py (the deep module). These re-exports keep the old
# call sites and the test surface working unchanged; every stderr signal is
# identical. post_login_gates is the post-sign-in 2FA + CAPTCHA check that
# used to sit inline in cmd_fill.
from apply.act.auth_flow import (
    handle_login_wall as _handle_login_wall,
    login_check as _login_check,
    check_account_created as _check_account_created,
    fill_signin_form as _fill_signin_form,
    reopen_signin_form as _re_open_signin_form,
    post_login_gates,
)
