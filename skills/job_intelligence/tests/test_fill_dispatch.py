...
class FillAnswersPersisted(unittest.TestCase):
    ...
    def _patch_fill(self, saved_states):
        ...
        patches = [
            ...
            patch("apply.common.page_helpers.handle_captcha", return_value=False),
            patch("apply.common.page_helpers.check_captcha", return_value=False),
...
class UploadFieldSelection(unittest.TestCase):
    """Cover-overwrite regression: only real file inputs/dropzones take
    the upload path; cover fields must never overwrite the resume input."""

    def _field(self, tag, ftype, label, fid="", name=""):
        return {"tag": tag, "type": ftype, "label": label, "id": fid, "name": name,
